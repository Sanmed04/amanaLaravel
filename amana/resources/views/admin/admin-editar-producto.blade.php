...
<div class="flex justify-center items-center lg:block">
    ...
    <form class="max-w-sm mx-auto mt-10 px-4" action="{{ route('admin.admin-actualizar-producto', $producto) }}" method="POST"
        ...
        <div class="relative z-0 w-full mb-5 group">
            ...
            <textarea id="descripcion" rows="4" name="descripcion" value="{{ old('descripcion', $producto->descripcion) }}"
        ...
        <div class="relative z-0 w-full mb-5 group">
            <input type="number" name="descuento" id="descuento" value="{{ old('descuento', $producto->descuento) }}"
                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                placeholder=" " />
            <label for="descuento"
        ...
        <div class="relative border shadow-xl mb-4">
            @if ($producto->stock == 0)
        ...
        <select id="categoria_id" name="categoria_id"
            ...
            @foreach ($categorias as $categoria)
                <option @selected(old('categoria_id', $producto->categoria_id) == $categoria->id) value="{{ $categoria->id }}">
                    {{ $categoria->nombre }}
                </option>
            @endforeach
        </select>
